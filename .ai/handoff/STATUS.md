> Note (2026-07-19, claude-opus-4-8): Aligned the AAHP v3.8.0 conformance PR (#75) with the gemini-code-assist review. Applied: filled the scaffolded [PROJECT] placeholder in LOG-ARCHIVE.md with aahp-runner; restored the MANIFEST.json project field to aahp-runner (the CLI regen had written the temp working-dir name); preserved the tasks object and next_task_id 4 (T-001 to T-003, with their github_issue links) which a plain manifest regen strips; and corrected the PR body which wrongly said GitHub Actions is off org-wide (homeofe Actions is on). Rejected: the GROUNDING.md grammar edit (canonical AAHP v3.8.0 template wording is the source of truth and is fixed upstream, not per repo) and the TRUST.md Provenance value changes (migrate-grounding seeds "-"/unknown placeholders on purpose; no claim was re-verified in this mechanical PR, so asserting a specific provenance would fabricate verification).

> Note (2026-07-18, claude-opus-4-8): Adopted CLI-based AAHP conformance (v3.8.0). Pinned @elvatis_com/aahp at exact 3.8.0 in devDependencies plus a root aahp.config.json (pinnedDep + em-dash forbidden pattern); added the Grounded Reflection Layer (GROUNDING.md and a Provenance column in TRUST.md); switched .github/workflows/aahp-verify.yml from the vendored bash script to the pinned CLI (npm ci then npx --no-install aahp verify/doctor), dependabot exemption preserved; and stopped vendoring the package-provided gate scripts (_aahp-lib.sh, aahp-manifest.sh, lint-handoff.sh, verify-handoff.sh, install-hooks.sh, verify-hooks.sh, hooks/pre-commit, hooks/pre-push) which the pinned CLI now supplies. The repo-specific scripts/validate-pii-allowlist.py is kept. MANIFEST regenerated.
